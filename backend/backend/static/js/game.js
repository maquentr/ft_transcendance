...
import { createButton, createMenu, createInput, createLabel, createText } from "./utilsGame.js";


function createKeyInfoPower(nbOfPlayers, team)
{
    let keyPower;
    ...
    {
        if (team)
            keyPower =  createText('[INPUT] Team 1: Q | Team 2 : Right arrow');
        else
            keyPower = createText('[INPUT] Player1: Z | Player2: C | Player3: M | Player4: . ');
    }
    else
    {
        keyPower = createText('[INPUT POWER UP] Player1: Z | Player2: Right arrow');
    }
    return (keyPower);
}


function createKeyInfoMove(nbOfPlayers)
{
    let keyMove;

    if (nbOfPlayers ===  4)
        keyMove = createText('[INPUT] Player1: W-S | Player2: R-F | Player3: U-J | Player4: O-L');
    else
        keyMove = createText('[INPUT] Player1: W-S | Player2: Up arrow - Down arrow');
    return (keyMove);
}
...
function preLobbyCustom(container, nbOfPlayers, team)
{
    ...
    let keyMove =  createKeyInfoMove(nbOfPlayers);
    let keyPower =  createKeyInfoPower(nbOfPlayers, team);
    ...
    if (team)
    {
        ...
        container.appendChild(keyMove);
        container.appendChild(keyPower);
    ...
    else
    {
        ...
        container.appendChild(launchButton);
        container.appendChild(keyMove);
        container.appendChild(keyPower); //same menu for 1v1 custom and 4pCustom ? so this text must not be shown ? 
        launchButton.onclick = function()
        {
            if (nbOfPlayers === 2)
            {
...
function preLobbyClassic(container, nbOfPlayers, team)
{
    ...
    container.appendChild(scoreToReach);
    container.appendChild(launchButton);
    container.appendChild(keyMove); //same question as previous
