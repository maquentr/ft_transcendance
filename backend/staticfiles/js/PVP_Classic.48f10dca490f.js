...
// Need to retrieve the name of both player 
...
let Game = {
    initialize: function (goal, player1Name = null, player2Name = 'Player 2') {
        ...
        // Récupérer le nom du joueur connecté s'il n'est pas fourni
        if (!player1Name) {
            let parag = document.getElementById('displayName');
            if (parag && parag.textContent.trim() !== '') {
                player1Name = parag.textContent.trim();
            } else {
        ...
        this.player1 = Player.new.call(this, player1Name, 'left');
        this.player2 = Player.new.call(this, player2Name, 'right');
...
async function start1v1Game(goal, player1Name = null, player2Name = null) {
    Pong = Object.assign({}, Game);

    Pong.initialize(goal, player1Name, player2Name);

    let winner = await Pong.onGameOver();

    if (player1Name && player2Name) {
        winner += ";" + player1Name + ";" + player2Name;
        quitGame('callback-1v1classic-online', winner);
    } else {
        quitGame('callback-1v1classic', winner);
    }
}
