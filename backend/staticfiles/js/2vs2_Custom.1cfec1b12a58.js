...
let Game = {
    initialize: function(goal){
        ...
        this.player1 = Player.new.call(this, paddleColors[0], userName, 'left', 'up');
        this.player2 = Player.new.call(this, paddleColors[0], 'Sarkozy', 'left', 'down');
        this.player3 = Player.new.call(this, paddleColors[1], 'Balkany', 'right', 'up');
        this.player4 = Player.new.call(this, paddleColors[1], 'Macron', 'right', 'down');
        this.ball = Ball.new.call(this, ballColor);

        this.team1 = Team.new.call(this, '1');
        this.team2 = Team.new.call(this, '2');
        assignPowerUps(this.team1, powerUp[0]);
        assignPowerUps(this.team2, powerUp[1]);
    ...
    UsePower: function (team)
    {
        if (team.powerUp >= 20)
        {
            if (team.powerChoose === power.DOUBLEPTS)
                team.doublePts = true;
            else if (Pong.team1 === team)
            {
                if (team.powerChoose === power.BIGPADDLE)
                {
                    Pong.player1.height += (Pong.player1.initialHeight / 4);
                    Pong.player2.height += (Pong.player2.initialHeight / 4);
                }
                else if (team.powerChoose === power.PADSPEED)
                {
                    Pong.player1.speed += 2;
                    Pong.player2.speed += 2;
                }
                else if (team.powerChoose === power.SMALLPADDLE)
                {
                    Pong.player3.height -= (Pong.player3.initialHeight / 4);
                    Pong.player4.height -= (Pong.player4.initialHeight / 4);
                }
            }
            else if (Pong.team2 === team)
            {
                if (team.powerChoose === power.BIGPADDLE)
                {
                    Pong.player3.height += (Pong.player3.initialHeight / 4);
                ...
                {
                    Pong.player3.speed += 2;
                    Pong.player4.speed += 2;
                }
                else if (team.powerChoose === power.SMALLPADDLE)
                {
                    Pong.player1.height -= (Pong.player1.initialHeight / 4);
    ...
    // Draw the winning score (center)
       this.context.fillText(
        ...
        this.context.fillText(
            'Power Up : ' + this.team1.powerUp.toString() + ' /20',
            (this.canvas.width / 8),
            (this.canvas.height) - (this.canvas.height / 25)
            );
            
            this.context.fillText(
                'Power Up : ' + this.team2.powerUp.toString() + ' / 20',
    ...
    listen: function () {
        ...
        document.addEventListener('keydown', function (key)
        {
            ...
            if (key.keyCode === 79) Pong.player3.move = DIRECTION.UP;
            if (key.keyCode === 74) Pong.player4.move = DIRECTION.UP;
            ...
            if (key.keyCode === 76 ) Pong.player3.move = DIRECTION.DOWN;
            if (key.keyCode === 78 ) Pong.player4.move = DIRECTION.DOWN;

            // Handle team PowerUp use
            if (key.keyCode === 81) Pong.UsePower(Pong.team1);
            else if (key.keyCode === 191) Pong.UsePower(Pong.team2);
        });
        ...
        { 
            ...
            else if (key.keyCode === 74 || key.keyCode === 78)
...
async function start2v2CustomGame(goal)
{
    ...
    quitGame('callback-2v2custom', winner);
}

export {start2v2CustomGame};